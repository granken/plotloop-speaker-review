(function (root, factory) {
  ...
  function normalizeMapping(mapping) {
    let source = mapping || {};
    let name = clean(source.name);
    let action = validChoice(source.action, ACTIONS, "keep");
    let confidence = validChoice(source.confidence, CONFIDENCES, "low");
  ...
  function meetingKey(meeting) {
    let item = normalizeMeeting(meeting);
    return [item.date, item.time, item.file_stem].join("::");
  }

  function parseReviewPayload(input) {
    let payload = typeof input === "string" ? JSON.parse(input) : input;
    ...
    sourceMeetings.forEach(function (source) {
      let item = normalizeMeeting(source);
      let key = meetingKey(item);
      if (!seen[key]) {
        seen[key] = true;
        meetings.push(item);
      }
  ...
  function applyNameDecision(mapping, nextName) {
    let item = Object.assign({}, mapping);
    let selectedName = clean(nextName);
  ...
  function buildPayload(activeMeeting, reviewedMeetings, now) {
    let reviewed = Array.isArray(reviewedMeetings) ? reviewedMeetings : [];
    return {
      type: "speaker-review",
      version: 2,
  ...
  function formatMoment(meeting) {
    let date = clean(meeting.date) || "日期待补";
    let time = clean(meeting.time) || "时间待补";
    return date + " " + time;
  }
